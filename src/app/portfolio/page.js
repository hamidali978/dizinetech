...
const Portfolio = () => {
    return (
        <>
            ...
            <Breadcrum pageTitle={"Portfolio"} pagename={"Portfolio"} />
            <div className="portfolio-grid-page pt-150 mb-120" id="scroll-section">
                <div className="container">
                    <div className="row g-4 mb-70">
                        {portfolioData.map((item) => (
                            <div key={item.id} className="col-lg-6">
                                <div className={`portfolio-card magnetic-item ${item.id % 2 === 0 ? 'mt-30' : ''}`}>
                                    <div className="card-img-and-content-wrap">
                                        <img src={item.image} alt={item.title} />
                                        <div className="card-content">
                                            <span>Client: {item.client}</span>
                                            <h4><Link href={`/portfolio/${item.id}`}>{item.title}</Link></h4>
                                            <ul className="tag-list">
                                                {item.tags.map((tag, index) => (
                                                    <li key={index}><Link href={`/portfolio/${item.id}`}>{tag}</Link></li>
                                                ))}
                                            </ul>
                                        </div>
                                    </div>
